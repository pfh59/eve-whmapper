...
namespace WHMapper.Services.EveAPI.Search
{
    public class SearchServices : EveApiServiceBase, ISearchServices
    {
        ...
        public async Task<Result<SearchAllianceResults>> SearchAlliance(string searchValue, bool isStrict = false)
        {
            ...
            {
                return await base.Execute<SearchAllianceResults>(RequestSecurity.Authenticated, RequestMethod.Get, string.Format("/characters/{0}/search/?datasource=tranquility&search={1}&categories=alliance&strict={2}", UserToken.AccountId, searchValue, isStrict));
            }
        ...
        public async Task<Result<SearchCharacterResults>> SearchCharacter(string searchValue, bool isStrict = false)
        {
            if (this.UserToken != null)
            {
                return await base.Execute<SearchCharacterResults>(RequestSecurity.Authenticated, RequestMethod.Get, string.Format("/characters/{0}/search/?datasource=tranquility&search={1}&categories=character&strict={2}", UserToken.AccountId, searchValue, isStrict));
            }
        ...
        public async Task<Result<SearchCoporationResults>> SearchCorporation(string searchValue, bool isStrict = false)
        {
            if (this.UserToken != null)
            {
                return await base.Execute<SearchCoporationResults>(RequestSecurity.Authenticated, RequestMethod.Get, string.Format("/characters/{0}/search/?datasource=tranquility&search={1}&categories=corporation&strict={2}", UserToken.AccountId, searchValue, isStrict));
            }
