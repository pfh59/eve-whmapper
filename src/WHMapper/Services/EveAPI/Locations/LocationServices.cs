...
namespace WHMapper.Services.EveAPI.Locations
{
    public class LocationServices : EveApiServiceBase, ILocationServices
    {
        ...
        public async Task<Result<EveLocation>> GetLocation()
        {
            ...
            {
                return await base.Execute<EveLocation>(RequestSecurity.Authenticated, RequestMethod.Get, string.Format("/characters/{0}/location/?datasource=tranquility", UserToken.AccountId));
            }
        ...
        public async Task<Result<Ship>> GetCurrentShip()
        {
            if (this.UserToken != null)
            {
                return await base.Execute<Ship>(RequestSecurity.Authenticated, RequestMethod.Get, string.Format("/characters/{0}/ship/?datasource=tranquility", UserToken.AccountId));
            }
