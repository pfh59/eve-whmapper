namespace WHMapper.Shared.Services.EveAPI.Alliances
{
    public class AllianceServices : EveApiServiceBase, IAllianceServices
    {
        ...
        {
            return await Execute<Models.DTO.EveAPI.Alliance.Alliance>(RequestSecurity.Public, RequestMethod.Get, string.Format("/v4/alliances/{0}/?datasource=tranquility", alliance_id));
        }

        public async Task<int[]?> GetAlliances()
        {
            return await Execute<int[]>(RequestSecurity.Public, RequestMethod.Get, "/v2/alliances/?datasource=tranquility");
        }
    }
}